...
namespace TopGame.Core.Infrastructure
{
    public class AppTypeFinder : ITypeFinder
    {
        ...
        {
            foreach (var assembly in App.GetAssemblies().Where(assembly => Matches(assembly.FullName)).Where(assembly => !addedAssemblyNames.Contains(assembly.FullName)))
            {
        ...
        {
            foreach (var assembly in AssemblyNames.Select(Assembly.Load).Where(assembly => !addedAssemblyNames.Contains(assembly.FullName)))
            {
                assemblies.Add(assembly);
                addedAssemblyNames.Add(assembly.FullName);
            }
        ...
        protected virtual bool DoesTypeImplementOpenGeneric(Type type, Type openGeneric)
        {
            try
            {
                ...
                return (from implementedInterface 
                        in type.FindInterfaces((objType, objCriteria) => true, null) 
